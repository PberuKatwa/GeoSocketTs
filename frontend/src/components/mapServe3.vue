<template>
  <div class="map-interface-layout">
    <MapSidebar :actions="sidebarActions" />
...
<script setup lang="ts">
  ...
  // ⭐️ Computed property to define the actions for the sidebar
  const sidebarActions = computed<MapSide[]>(() => [
    { label: "Add Center", handler: addCenter },
    { label: "Add Target", handler: addTarget },
    { label: "Request Route", handler: requestRoute },
    { label: "Start Simulation", handler: startSimulation },
    { label: "Stop Simulation", handler: stopSimulation },
  ]);

</script>

<style>
/* Add a layout style to arrange the sidebar and the map */
